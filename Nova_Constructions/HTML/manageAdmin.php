<?php
    ...
    if(isset($_POST['submit']))
    {
      ...
      $sql = "INSERT INTO admindetails (fName, lName, email, phone, username, password, code) VALUES ('$first_name', '$last_name', '$email', '$phone', '$username', '$password', '$code')";
...
<html>
  ...
  <head>
    <title>Admin Manage | Nova Constructions</title>
  ...
  <body>
    ...
    <div class="adminReg">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" onsubmit="return userRegisterPasswordCheck()">
          ...
          <input type="text" id="code" name="code" placeholder="ADMIN CODE" pattern="[A-Z][A-Z]\d{3}" title="Admin code should be a 5 letter code with 2 uppercase letters in the beginning and 3 numbers in the end." required>
                ...
                while($row = $result->fetch_assoc()) {
                   echo "
                    <tr>
                        ...
                        <td>$row[username]</td>
                        <td rowspan=\"2\"><a href=\"adminUpdate.php?id=$row[no]\"><button>Update</button></a>
                            <a href=\"adminDelete.php?id=$row[no]\"><button>Delete</button></a>
                        </td>
                    </tr>
                    <tr>
